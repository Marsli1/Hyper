...
if (message.content === '<@429500693741568020>') {
  const embed = new Discord.RichEmbed()
  .setTitle('Ben MasterGamerTR:')
  .setDescription('Prefixim: `!`')
  .setFooter('MasterGamerTR')
  .setColor('RANDOM')
 message.reply(embed)
}
});
...
client.on('guildCreate', async guild => {
  const girismesaj = [
    ...
    'Bu bot **<@529037946746306581>** tarafından geliştirilmektedir.',
...
client.on("guildCreate", async guild => {
  ...
  console.log(`${guild.name} with invite: https://discord.gg/N7Kr6gV`)
});
